...
function moveTo(id) {
  ...
  else window.scrollTo(0, document.getElementById(id).offsetTop - 70);
...
function removeClass(element, name) {
  ...
  while (arr.indexOf(name) > -1) arr.splice(arr.indexOf(name), 1);
...
function showReviewSlides(n) {
  ...
  if (n > review_slides.length - 3) reviewSlideIndex = 0;

  if (n < 0) reviewSlideIndex = reviewSlideIndex - 3;
  ...
  addClass(review_slides[reviewSlideIndex + 1], "res-show");
  removeClass(review_slides[reviewSlideIndex + 2], "hide");
  addClass(review_slides[reviewSlideIndex + 2], "res-show");
}
